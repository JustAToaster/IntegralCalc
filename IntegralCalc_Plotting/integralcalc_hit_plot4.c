...
//GPU plotting by writing to an uchar4 array
//Useless and inefficient vectorization
...
int replaceFunction(char * f){
	...
	fclose(code);
	fclose(temp);
	remove("montecarlo4.ocl");
	rename("temp.txt", "montecarlo4.ocl");
	return 1;
}
...
cl_event montecarlo(cl_kernel montecarlo_k, cl_command_queue que, cl_mem d_hits, 
	...
	cl_mem d_plot, cl_uint seed1, cl_uint seed2, cl_uint seed3, 
	cl_uint seed4, cl_int plotWidth, cl_int plotHeight, cl_event prev_evt){
...
int main(int argc, char* argv[]){
	...
	cl_program prog = create_program("montecarlo4.ocl", ctx, d);
	cl_int err;
	...
	cl_kernel montecarlo_k = clCreateKernel(prog, "montecarlo_hit_plot", &err);
	...
	*h_null = 0;
	...
	cl_event montecarlo_evt, initPlot_evt, readHits_evt, readPlot_evt;
	...
	//createBlankImage((uchar*)plotInfo.data, plotInfo.data_size);
	...
	montecarlo_evt = montecarlo(montecarlo_k, que, d_hits, leftLimit, rightLimit, 
		h_max.y, npoints/8, d_plot, seed1, seed2, seed3, seed4, plotInfo.width, plotInfo.height, initPlot_evt);
	...
	double total_time_ms = runtime_max_ms + runtime_montecarlo_ms + runtime_readHits_ms + runtime_readPlot_ms;
