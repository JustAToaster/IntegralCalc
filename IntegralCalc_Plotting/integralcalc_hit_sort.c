...
//GPU plotting in an uchar4 array, after sorting (with GPU) the couples (index, color value) by index
//Plotting is more efficient but sorting is, of course, expensive even on GPU
...
cl_event montecarlo(cl_kernel montecarlo_k, cl_command_queue que, cl_mem d_hits, 
	float leftLimit, float rightLimit, float funcMax, cl_int _gws, 
...
cl_event plotter(cl_kernel plotter_k, cl_command_queue que, cl_mem d_plot, cl_mem d_plotIndexes, int npoints){

	const size_t gws[] = { npoints/2 };

	cl_event plotter_evt;
	cl_int err;

	cl_uint i = 0;
	err = clSetKernelArg(plotter_k, i++, sizeof(d_plot), &d_plot);
	ocl_check(err, "set plotter arg ", i-1);
	err = clSetKernelArg(plotter_k, i++, sizeof(d_plotIndexes), &d_plotIndexes);
	ocl_check(err, "set plotter arg ", i-1);

	err = clEnqueueNDRangeKernel(que, plotter_k, 1, NULL, gws, NULL,
		0, NULL, &plotter_evt);
	ocl_check(err, "enqueue plotter");
	...
	cl_mem d_v1, cl_int nels, cl_event init_event)
{
	...
	const size_t lws[] = { _lws };
	//printf("init gws e workitem : %d | %u = %zu  %li\n", nels, _lws, gws[0],workitem);
...
int main(int argc, char* argv[]){
	...
	cl_kernel montecarlo_k = clCreateKernel(prog, "montecarlo_hit_sort", &err);
	ocl_check(err, "create kernel montecarlo");

	cl_kernel plotter_k = clCreateKernel(prog, "plotter", &err);
	ocl_check(err, "create kernel plotter");
	...
	*h_null = 0;
	...
	cl_event montecarlo_evt, plot_evt, initPlot_evt, readHits_evt, readPlot_evt;
	...
	cl_mem d_plotIndexes = clCreateBuffer(ctx,
		CL_MEM_READ_WRITE,
		npoints*sizeof(cl_int), NULL,
		&err);
	ocl_check(err, "create buffer d_plotIndexes");
	...
	montecarlo_evt = montecarlo(montecarlo_k, que, d_hits, leftLimit, rightLimit, 
		h_max.y, npoints/2, d_plotIndexes, seed1, seed2, plotInfo.width, plotInfo.height);
	...
	cl_kernel sort_k = clCreateKernel(prog, "stable_local_count_sort_vectlmemV3", &err);
	...
	err = clGetKernelWorkGroupInfo(sort_k, d,
		...
		sizeof(sort_lws), &sort_lws, NULL);
	ocl_check(err, "preferred wg multiple for sort");

	cl_mem d_plotIndexes2 = clCreateBuffer(ctx,
		CL_MEM_READ_WRITE,
		npoints*sizeof(cl_int), NULL,
		&err);
	ocl_check(err, "create buffer d_plotIndexes");
	cl_event sort_evt, merge_evt1, merge_evt2;
	sort_evt = sortparallel(sort_k, sort_lws, que, d_plotIndexes, npoints, readHits_evt);
	int turn=0,pass=1;	
	double total_time_merge=0;
	int current_merge_size = sort_lws;
	merge_evt1=sort_evt;
	double merge_bw_gbs;
	
	while(current_merge_size<npoints){
		if(turn ==0){
			turn = 1;
			merge_evt2 = sortparallelmerge(sort_merge_k, sort_lws, que, d_plotIndexes, d_plotIndexes2, npoints, merge_evt1, current_merge_size);
			clWaitForEvents(1, &merge_evt2);

			const double runtime_merge_ms = runtime_ms(merge_evt2);
			total_time_merge += runtime_merge_ms;
			merge_bw_gbs = npoints*sizeof(cl_int)*log2(npoints)/1.0e6/runtime_merge_ms;
			//printf("merge_parziale_lws%i destinazione Sort2: %d int in %gms: %g GB/s %g GE/s\n",
			//		current_merge_size, npoints, runtime_merge_ms, merge_bw_gbs, (npoints)/1.0e6/runtime_merge_ms);
		
		}
		else{
			turn = 0;
			merge_evt1 = sortparallelmerge(sort_merge_k, sort_lws, que, d_plotIndexes2, d_plotIndexes, npoints, merge_evt2,current_merge_size);
	...
	if(turn == 0){
		plot_evt = plotter(plotter_k, que, d_plot, d_plotIndexes, npoints);
	}else{
		plot_evt = plotter(plotter_k, que, d_plot, d_plotIndexes2, npoints);
	}

	plotInfo.data = clEnqueueMapBuffer(que, d_plot, CL_TRUE,
		CL_MAP_READ,
		0, plotInfo.data_size,
		1, &plot_evt, &readPlot_evt, &err);
	...
	double runtime_plot_ms = runtime_ms(plot_evt);
	double runtime_readPlot_ms = runtime_ms(readPlot_evt);
	double total_time_ms = runtime_max_ms + runtime_montecarlo_ms + runtime_readHits_ms + runtime_sort_ms + total_time_merge + runtime_readPlot_ms;

	double max_bw_gbs = k*nsamples/1.0e6/runtime_max_ms;
	double initPlot_bw_gbs = plotInfo.data_size/1.0e6/runtime_initPlot_ms;
	double montecarlo_bw_gbs = ((npoints/2) * sizeof(cl_int) + 4*npoints*sizeof(uchar))/1.0e6/runtime_montecarlo_ms;
	const double sort_bw_gbs = ((npoints*sizeof(cl_int)*log2(npoints)/1.0e6/runtime_sort_ms)+merge_bw_gbs)/2;
	double readHits_bw_gbs = sizeof(cl_int)/1.0e6/runtime_readHits_ms;
	double plot_bw_gbs = (4*npoints*sizeof(cl_int))/1.0e6/runtime_plot_ms;
	...
	printf("sort: %d int (indexes) in %gms: %g GB/s %g GE/s\n",
		npoints, runtime_sort_ms+total_time_merge, sort_bw_gbs, (npoints)/1.0e6/runtime_sort_ms);
	printf("plotting : %d points in %gms: %g GB/s\n",
		npoints, runtime_plot_ms, plot_bw_gbs);
	...
	clReleaseMemObject(d_plotIndexes);
	clReleaseMemObject(d_plotIndexes2);
