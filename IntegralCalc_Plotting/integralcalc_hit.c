...
int main(int argc, char* argv[]){
	...
	*h_null = 0;
	...
	cl_event montecarlo_evt, readHits_evt, readPlot_evt;
	...
	uchar *plotData = (uchar*)plotInfo.data;
	createBlankImage(plotData, plotInfo.data_size);
	printf("Creating image %dx%d with data size %ld bytes\n", plotInfo.width, plotInfo.height, plotInfo.data_size);
	save_pam(imageName, &plotInfo);
	cl_mem d_plot = clCreateBuffer(ctx,
		CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
		2*npoints*sizeof(cl_int), NULL,
		&err);
	ocl_check(err, "create buffer d_plot");

	montecarlo_evt = montecarlo(montecarlo_k, que, d_hits, leftLimit, rightLimit, 
		h_max.y, npoints/2, d_plot, seed1, seed2, plotInfo.width, plotInfo.height);
	...
	cl_int * h_plot = clEnqueueMapBuffer(que, d_plot, CL_TRUE,
		CL_MAP_READ,
		0, 2*npoints*sizeof(cl_int),
		1, &readHits_evt, &readPlot_evt, &err);
	ocl_check(err, "enqueue map d_plot");

	int x1, x2, y1, y2, hit_data, pixel1, pixel2;

	clock_t start_plot, end_plot;

	start_plot = clock();
	for(int i=0; i<npoints/2; i+=4){
		x1 = (h_plot[i] < plotInfo.width) ? h_plot[i] : plotInfo.width-1;
		x2 = plotInfo.width-x1;
		y1 = (h_plot[i+1] < plotInfo.height) ? h_plot[i+1] : plotInfo.height-1;
		y2 = (h_plot[i+2] < plotInfo.height) ? h_plot[i+2] : plotInfo.height-1;
		hit_data = h_plot[i+3];
		pixel1 = (plotInfo.width*y1+x1)<<2;	//index of the red channel
		pixel2 = (plotInfo.width*y2+x2)<<2;
		if(hit_data == 3){	//Both hit
			//Erase red and green channel
			plotData[pixel1]=0;	
			plotData[pixel1+1]=0;
			plotData[pixel2]=0;	
			plotData[pixel2+1]=0;
		}
		else if(hit_data==2){	//The second one hits
			//erase pixel1 green and blue channel
			plotData[pixel1+1]=0;	
			plotData[pixel1+2]=0;
			//erase pixel2 red and green channel
			plotData[pixel2]=0;	
			plotData[pixel2+1]=0;
		}
		else if(hit_data==1){	//The first one hits
			//erase pixel2 green and blue channel
			plotData[pixel2+1]=0;	
			plotData[pixel2+2]=0;
			//erase pixel1 red and green channel
			plotData[pixel1]=0;	
			plotData[pixel1+1]=0;
		} else{	//neither hit
			//erase green and blue channel for both
			plotData[pixel1+1]=0;	
			plotData[pixel1+2]=0;
			plotData[pixel2+1]=0;	
			plotData[pixel2+2]=0;
	...
	double runtime_plotting_ms = (end_plot - start_plot)*1.0e3/CLOCKS_PER_SEC;
	double total_time_ms = runtime_max_ms + runtime_montecarlo_ms + runtime_readHits_ms 
		+ runtime_readPlot_ms + runtime_plotting_ms;

	double max_bw_gbs = k*nsamples/1.0e6/runtime_max_ms;
	double montecarlo_bw_gbs = (*h_hits * sizeof(cl_int) + 2*npoints*sizeof(cl_int))/1.0e6/runtime_montecarlo_ms;
	...
	double plotting_bw_gbs = npoints*sizeof(cl_int)/1.0e6/runtime_plotting_ms;
	...
	printf("read plot data : %d int in %gms: %g GB/s\n",
		2*npoints, runtime_readPlot_ms, readPlot_bw_gbs);
	printf("plotting (host) : %d (int,int) in %gms: %g GB/s\n",
		npoints, runtime_plotting_ms, plotting_bw_gbs);
	printf("\nTotal time: %g ms.\n", total_time_ms);
	//printf("Total:%g;%g\n", intervalSize, runtime_max_ms);

	err = clEnqueueUnmapMemObject(que, d_hits, h_hits, 0, NULL, NULL);
	ocl_check(err, "unmap hits");
	err = clEnqueueUnmapMemObject(que, d_plot, h_plot, 0, NULL, NULL);
