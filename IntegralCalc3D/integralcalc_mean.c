...
cl_event montecarlo(cl_kernel montecarlo_k, cl_command_queue que,
	...
	float leftLimitY, float rightLimitY, cl_uint seed1, cl_uint seed2,
	cl_uint seed3, cl_uint seed4, int npoints){
...
//The volume beneath the curve is approximately the mean of the function values at the random points * the area of the region of integration
double computeIntegral(const double sumValue, const double rectArea, const int npoints){
 	return rectArea * (sumValue / npoints);
}
...
int main(int argc, char* argv[]){
	...
	if(argc <= 6){
		...
		printf("%s 'x*log(y)' 0.0 1.0 0.0 1.0 8192 -0.5\n", argv[0]);
		exit(0);
	}
	else{
		npoints = atoi(argv[6]);
	...
	montecarlo_evt = montecarlo(montecarlo_k, que, d_sum1, leftLimitX, rightLimitX, 
		leftLimitY, rightLimitY, seed1, seed2, seed3, seed4, npoints);
	...
	double calculatedValue = computeIntegral(sum_result, rectArea, npoints);
	printf("(%s, %s)∫(%s, %s)∫[%s] dxdy = %g\n\n", argv[2], argv[3], argv[4], argv[5], argv[1], calculatedValue);
