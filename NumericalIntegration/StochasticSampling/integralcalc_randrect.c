...
cl_event vecinit(cl_kernel vecinit_k, cl_command_queue que,
	cl_mem d_v, float leftLimit, float rightLimit, cl_uint seed1, cl_uint seed2, cl_uint seed3, cl_uint seed4, int nsamples){
...
//Setting up the kernel to compute the rectangles areas
cl_event intcalc(cl_kernel intcalc_k, cl_command_queue que,
	cl_mem d_v1, cl_mem d_v2, int nsamples){

	const size_t gws[] = { nsamples-1 };	//there are nsamples-1 rectangles
...
//Setting up the kernel to sum all the areas
...
int main(int argc, char* argv[]){
	...
	cl_kernel intcalc_k = clCreateKernel(prog, "intcalc_randrect", &err);
	...
	intcalc_evt = intcalc(intcalc_k, que, d_v1, d_v2, nsamples);
